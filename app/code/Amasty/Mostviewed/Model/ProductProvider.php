...
class ProductProvider
{
    ...
    private function applyBoughtTogether(Collection $collection, Product $product, Group $group)
    {
        ...
        $db = $collection->getConnection();
        $storeId = $this->storeManager->getStore()->getId();
        $productIds = $this->getProductIdsByType($product);
        ...
        $orderSelect = $db->select()
            ->from(['order_item' => $tbl], ['order_item.order_id'])
            ->where('order_item.product_id IN(?)', $productIds)
            ->where('order_item.store_id = ?', $storeId)
            ->where('TO_DAYS(NOW()) - TO_DAYS(order_item.created_at) <= ?', $this->config->getGatheredPeriod());

        $productSelect = $db->select()->from(
            ['order_item' => $tbl],
        ...
        $this->addOrderStatusFilter($productSelect);
        $data = $db->fetchAll($productSelect);
